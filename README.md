# Gerard Hales — Portfolio

Personal portfolio website for Gerard Hales, Business Intelligence Developer at UBS and founder of Elite BI Solutions.

Live at: [halesgerard.github.io](https://halesgerard.github.io)

## About

This site showcases 9+ years of BI & data development experience across banking, HR analytics and engineering. Built as a single-page portfolio covering services, selected projects, career history, tech stack and contact details.

## Tech

- Designed and built with [Claude](https://claude.ai) (Anthropic)
- Self-contained HTML — no frameworks, no build step
- Fonts: Space Grotesk · IBM Plex Sans · IBM Plex Mono
- Icons: Iconify API

## Contact

📧 hales.gerard@gmail.com  
🔗 [linkedin.com/in/gerard-hales](https://pl.linkedin.com/in/gerard-hales)


---
*© 2026 Gerard Hales · Elite BI Solutions. All rights reserved.*
